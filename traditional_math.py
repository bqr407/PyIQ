...
class Game(Frame):
    ...
    def drawMenu(self):
        ...
        self.canvas.create_text((self.canvas_width * .5, self.canvas_height * .4), text="PyIQ - Traditional Math",
    ...
    def drawGame(self):
        ...
        def displayProblem():
            ...
            if self.problemNumber <= self.numProblems:
                ...
                # Display option A
                self.canvas.create_image(self.canvas_width * .35, self.canvas_height * .7, image=self.upImg,
                                         tags='problem')
                self.canvas.create_text((self.canvas_width * .35, self.canvas_height * .8), text=str(self.optionArr[0]),
                                        font=("Arial", 30), tags='problem')
                # Display option B
                self.canvas.create_image(self.canvas_width * .45, self.canvas_height * .7, image=self.rightImg,
                                         tags='problem')
                self.canvas.create_text((self.canvas_width * .45, self.canvas_height * .8), text=str(self.optionArr[1]),
                                        font=("Arial", 30), tags='problem')
                # Display option C
                self.canvas.create_image(self.canvas_width * .55, self.canvas_height * .7, image=self.leftImg,
                                         tags='problem')
                self.canvas.create_text((self.canvas_width * .55, self.canvas_height * .8), text=str(self.optionArr[2]),
                                        font=("Arial", 30), tags='problem')
                # Display option D
                self.canvas.create_image(self.canvas_width * .65, self.canvas_height * .7, image=self.downImg,
                                         tags='problem')
                self.canvas.create_text((self.canvas_width * .65, self.canvas_height * .8), text=str(self.optionArr[3]),
                                        font=("Arial", 30), tags='problem')
                self.onProblem = True
        ...
        def genProblem():
            ...
            if self.problemNumber < self.numProblems:  # If the player still has more problems left
                ...
                if type == 1:
                    ...
                    ans = x + y
                    problem = str(x) + "+" + str(y)
                    optionArr = [ans, ans + random.randint(-20, 20), ans + random.randint(-20, 20),
